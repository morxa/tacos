...
std::unique_ptr<automata::ata::Formula> create_contains(logic::TimeInterval)
{
	// TODO implement
	return std::make_unique<automata::ata::TrueFormula>();
}

std::unique_ptr<automata::ata::Formula> create_negated_contains(logic::TimeInterval)
{
	// TODO implement
	return std::make_unique<automata::ata::FalseFormula>();
}

std::unique_ptr<automata::ata::Formula>
init(const logic::MTLFormula &formula, const logic::AtomicProposition &ap)
{
	...
	case logic::LOP::LDUNTIL:
		return std::make_unique<automata::ata::ResetClockFormula>(
		  // TODO insert proper location formula
		  std::make_unique<automata::ata::LocationFormula>("formula"));
	case logic::LOP::LAND:
		return std::make_unique<automata::ata::ConjunctionFormula>(
		  init(formula.get_operands().front(), ap), init(formula.get_operands().back(), ap));
	case logic::LOP::LOR:
		return std::make_unique<automata::ata::DisjunctionFormula>(
		  init(formula.get_operands().front(), ap), init(formula.get_operands().back(), ap));
	case logic::LOP::AP:
		if (formula == ap) {
			return std::make_unique<automata::ata::TrueFormula>();
...
automata::ata::AlternatingTimedAutomaton
translate(const logic::MTLFormula &input_formula)
{
	...
	auto untils      = formula.get_subformulas_of_type(logic::LOP::LUNTIL);
	auto dual_untils = formula.get_subformulas_of_type(logic::LOP::LDUNTIL);
	locations.insert({{"phi_i"}});
	auto accepting_locations = formula.get_subformulas_of_type(logic::LOP::LDUNTIL);
	std::set<automata::ata::Transition> transitions;
	for (const auto &symbol : alphabet) {
		// TODO insert the actual formula (need templated ATAs for this)
		transitions.insert(automata::ata::Transition(
		  "phi_i", symbol.ap_, std::make_unique<automata::ata::LocationFormula>("formula")));

		for (const auto &until : untils) {
			auto transition_formula = std::make_unique<automata::ata::DisjunctionFormula>(
			  std::make_unique<automata::ata::ConjunctionFormula>(init(until.get_operands().back(),
			                                                           symbol),
			                                                      create_contains(until.get_interval())),
			  std::make_unique<automata::ata::ConjunctionFormula>(
			    init(until.get_operands().front(), symbol),
			    // TODO insert proper location formula
			    std::make_unique<automata::ata::LocationFormula>("until")));
			transitions.insert(
			  // TODO insert proper location formula
			  automata::ata::Transition("until", "symbol", std::move(transition_formula)));
		}
		for (const auto &dual_until : dual_untils) {
			std::make_unique<automata::ata::DisjunctionFormula>(
			  std::make_unique<automata::ata::ConjunctionFormula>(
			  ...
			  std::make_unique<automata::ata::ConjunctionFormula>(
			    ...
			    // TODO insert proper location formula
			    std::make_unique<automata::ata::LocationFormula>("dual_until")));
		}
	}
	return automata::ata::AlternatingTimedAutomaton(
	  {"a", "b"}, // TODO get alphabet from formula, needs templated ATAs
	  "phi_i",
	  {""}, // convert accepting_locations,
