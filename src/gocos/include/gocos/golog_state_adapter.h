/***************************************************************************
 *  golog_state_adapter.h - Generate successors of Golog configurations
 *
...
/** @brief Compute all successors for one particular time successor and all possible transitions of
 * the Golog program, using fluent-based constraints. Compute the successors by following all
 * transitions in the program and ATA for one time successor and all actions that can be executed in
 * the program. To compute the ATA successor, get the currently satisfied fluents from the program
 * history and pass them to the ATA.
 */
template <>
class get_next_canonical_words<GologProgram, std::string, std::string, true, true>
{
public:
